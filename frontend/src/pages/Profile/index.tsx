...
export default function Profile() {
    ...
    return <section className="mx-auto w-full max-w-5xl px-4 py-10 md:px-8">
        ...
        </div> : <Card className="p-6"><p className="text-text-muted">Membro desde {new Date(profile.data_cadastro).toLocaleDateString("pt-BR")}.</p><div className="mt-4 flex flex-wrap gap-2"><Link to={`/chat?user=${profile.id}`}><Button type="button">Conversar</Button></Link>{friendIds.has(profile.id) ? <Button type="button" variant="secondary" onClick={() => removeFriend(profile.id)}>Remover amigo</Button> : <Button type="button" variant="secondary" onClick={() => addFriend(profile.id)}>Adicionar amigo</Button>}</div></Card>}

        <div className="mt-8 grid gap-6 lg:grid-cols-2"><Card className="p-6"><h2 className="mb-4 text-xl font-semibold">Amigos</h2>{friends.length === 0 ? <p className="text-text-muted">Nenhum amigo adicionado ainda.</p> : <ul className="space-y-3">{friends.map((friend) => <li key={friend.id} className="flex flex-wrap items-center justify-between gap-3 rounded-card border border-border p-3"><Link className="flex min-w-0 items-center gap-3" to={`/usuarios/${friend.id}`}><img src={friend.avatar_url} alt="" className="h-10 w-10 rounded-full object-cover" /><span><strong className="block">{friend.nome} {friend.sobrenome}</strong><span className="text-sm text-text-muted">{friend.online ? "Online" : "Offline"}</span></span></Link><div className="flex gap-2"><Link to={`/chat?user=${friend.id}`}><Button type="button" variant="secondary">Chat</Button></Link><Button type="button" variant="secondary" onClick={() => removeFriend(friend.id)}>Remover</Button></div></li>)}</ul>}</Card>
