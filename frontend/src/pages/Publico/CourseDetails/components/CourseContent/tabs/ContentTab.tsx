            ...
            <div key={module.id} className="border border-neutral-200 rounded-lg overflow-hidden bg-white">
              ...
              >
                <div className="flex flex-col gap-1">
                  <span className="font-bold text-neutral-900 text-lg">{module.title}</span>
                ...
                <div className="border-t border-neutral-200 divide-y divide-neutral-100">
                  {module.lessons.map((lesson) => (
                    <Link
                      to={`aulas?${lesson.id}`}
                      key={lesson.id}
                      className="p-4 pl-6 flex items-center justify-between hover:bg-neutral-50 transition-colors group cursor-pointer bg-white"
                    >
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <span className="text-xs text-neutral-400 w-10 text-right">{lesson.duration}</span>
                      </div>
                    </Link>
                  ))}
                </div>
